...
interface Props {
  ...
  label: string;
  onClick?: () => void;
  /** 本回合是否禁用此建筑（例如医疗已达每回合限制） */
  disabled?: boolean;
  /** 禁用时显示的手写提示文案 */
  disabledText?: string;
}

export default function Building({ kind, label, onClick, disabled = false, disabledText }: Props) {
  ...
  return (
    <button
      ...
      disabled={disabled}
      className={[
        'doodle-card group p-4 flex flex-col items-center gap-2 transition-transform',
        disabled
          ? 'opacity-40 cursor-not-allowed'
          : 'hover:-translate-y-1 active:translate-y-0',
      ].join(' ')}
    >
      <div className={!disabled && hover ? 'animate-wiggle' : ''}>
        <BuildingIcon kind={kind} />
      </div>
      <span className="font-hand text-xl">{label}</span>
      {disabled && disabledText
        ? <span className="font-sketch text-xs text-red-500">{disabledText}</span>
